...
deep_harness_analyze_one <- function(path){
  ...
  exec.path <- gsub("extdata/","",paste0("/",list.paths$root,"/",path_details$val,path_details$folder,path_details$package_name," ;"))
  ...
  package_path <- gsub("extdata/","",paste0("/",list.paths$root,"/",path_details$val,"testpkgs/",path_details$package_name))
  #print(var)
  system(var)
  ...
  for(function_name.i in unique(functions.list$funName)){ 
    ...
    if(function_name.i == paste0("rcpp_",fun)){
      ...
      for(argument.i in 1:nrow(functions.rows)){
        ...
        #print(arg.name)
        filepath=gsub("extdata/","",paste0("/",list.paths$root,"/",path_details$val,"testfiles/",path_details$package_name,arg.name))
